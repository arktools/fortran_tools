      SUBROUTINE OUTPT2
C
C     THIS SUBROUTINE WRITES THE HIGH LIFT AND CONTROL DATA
C
      COMMON /FLAPIN/ F(138)
...
C
C     SYMMETRICAL FLAP DATA DEFINITION
C
      DIMENSION DCDI(200), DCL(10), DCM(10), DCLMAX(10), DCDMIN(10),
     1          DCLAD(10), CHA(10), CHD(10)
      DIMENSION DELTA(10)
      EQUIVALENCE (DCDI(1),BODY(201)), (DCL(1),WING(201)),
     1          (DCM(1),WING(211)), (DCLMAX(1),WING(221)),
     2          (DCDMIN(1),WING(231)), (DCLAD(1),WING(241)),
     3          (CHA(1),WING(251)), (CHD(1),WING(261))
      EQUIVALENCE (DELTA(1),F(1)), (FTYPE,F(17))
C
C     CONTROL TAB DATA DEFINITION
C
     ...
     2            (DTT(1,1),BWHV(201))
C
C     ASYMMETRICAL FLAP DATA DEFINITION
C
     ...
     1          (DELTAD(1),F(1)), (STYPE,F(18)), (DELTAL(1),F(19)),
     2          (DELTAR(1),F(29)), (DELTAS(1),F(39)),
     3          (XSOC(1),F(49)), (HSOC(1),F(60)), (DDELTA(1),HT(201)),
     4          (CLD(1),HT(211)), (CND(1),HT(221))
C
C     TRIM DATA DEFINITION
C
      DIMENSION CLUT(20), CDUT(20), CMUT(20), DELT(20), DCLT(20),
     1          DCLMT(20), DCDIT(20), DCDMT(20), CHAT(20), CHDT(20)
      DIMENSION HMUNT(20), ALINT(20), CDTRM(20), CLTRM(20), CMTRM(20),
     1          HMTRM(20), CDWBT(20), CLWBT(20)
      EQUIVALENCE (CLUT(1),HT(201)), (CDUT(1),HT(221)),
     1          (CMUT(1),HT(241)), (DELT(1),VT(201)), (DCLT(1),VT(221)),
     2          (DCLMT(1),VT(241)), (DCDIT(1),VT(261)),
     3          (DCDMT(1),VT(281)), (CHAT(1),VT(301)),
     4          (CHDT(1),VT(321))
      EQUIVALENCE (HMUNT(1),HT(201)), (ALINT(1),HT(221)),
     1          (CDTRM(1),HT(241)), (CLTRM(1),HT(261)),
     2          (CMTRM(1),HT(281)), (HMTRM(1),HT(301)),
     3          (CDWBT(1),VT(201)), (CLWBT(1),VT(221))
...
C
     ...
     2  4H,1X ,   4H,F6.,   4H3   ,   4H,4X ,
     3  4H    ,   4H,F7.,   4H4   ,   4H,3X ,
     4  4H,1X ,   4H,F6.,   4H3   ,   4H,6X ,
     5  4H    ,   4H,F7.,   4H5   ,   4H,15X,
     6  4H,1P ,   4H E10,   4H.3  ,   4H,2X ,
     7  4H,1P ,   4H E10,   4H.3  ,   4H,2X ,
     8  4H,1P ,   4H E10,   4H.3  ,   4H)   /
      DATA IFOR / 5*7, 3*10 /
C
      DATA FOT /
     1  4H(0P ,   4H4X  ,   4H,1X ,   4H,F5.,   4H1,1X,   4H,1X ,
     2  4H,1X ,   4H,F6.,   4H3   ,   4H,3X ,
     3  4H,1X ,   4H,F6.,   4H3   ,   4H,3X ,
     4  4H    ,   4H,F7.,   4H4   ,   4H,2X ,
     5  4H,1X ,   4H,F5.,   4H1,1X,   4H,1X ,
     6  4H,1X ,   4H,F6.,   4H3   ,   4H,4X ,
     7  4H,1X ,   4H,F6.,   4H3   ,   4H,5X ,
     8  4H,1P ,   4H E9.,   4H2   ,   4H,2X ,
     9  4H,0P ,   4H F8.,   4H5,1X,   4H,2X ,
     A  4H,1P ,   4H E10,   4H.3  ,   4H,3X ,
     B  4H,1P ,   4H E10,   4H.3  ,   4H)   /
...
C
     ...
     1                TINF,RN,SREF,CBAR,BLRF,XCG,ZCG,DUM,
     2                DUM,DUM,NDMF,NAF)
C
          WRITE(6,1470)
          CALL SWRITE(8,FOR,34,IFOR,NDELTA,DELTA,DCL,DCM,DCLMAX,
     1               DCDMIN,DCLAD,CHA,CHD,X,X,X,X,X,X,NDMF,NAF)
...
C
     ...
     1                        DELTAS(I), CLD(I), CND(I), I=1,NDELTA)
               IF(TRANSN) WRITE(6,1290) (XSOC(I), HSOC(I),
     1                        DELTAS(I), CLD(I), I=1,NDELTA)
               GO TO 1050
 1040          WRITE(6,1260)
...
C
     ...
     1                     DELT,DCLT,DCLMT,DCDIT,DCDMT,CHAT,CHDT,
     2                     X,X,X,NDMF,NAF)
               IF(FLAG) CALL FLTCL(FOT,IFOT,NTRIM,CLUT,CDUT,CMUT,DELT,
     1                             DCLT,DCLMT,DCDIT,DCDMT,CHDT,CLWBT,
     2                             CDWBT,1)
...
C
     ...
     1                ,HT(I), HT(I+20), HT(I+40), HMUNT(I), ALINT(I)
     2                ,CDTRM(I), CLTRM(I), CMTRM(I), HMTRM(I)
                  IF(ALINT(I) .NE. -1000.) NTM = NTM+1
                  IF(ALINT(I) .EQ. -1000.)WRITE(6,1240) ALPHA(I)
     1                ,HT(I), HT(I+20), HT(I+40)
 1140          CONTINUE
               WRITE(6,1210)
               WRITE(6,1220)
               DO 1150 I=1,NTRIM
 ...
 1180 FORMAT(26X,76HWING-BODY-VERTICAL TAIL-ALL MOVABLE HORIZONTAL STABI
     1LIZER TRIM CONFIGURATION)
 1190 FORMAT(1H0,46H-----------------UNTRIMMED--------------------,
     134HHORIZONTAL STABILIZER COEFFICIENTS,45H----------------AT TRIM I
     2NCIDENCE------------/)
 1200 FORMAT(7H0 ALPHA,5X,2HCD,7X,2HCL,7X,2HCM,7X,2HHM,41X,5HALIHT,4X,
     12HCD,7X,2HCL,7X,2HCM,7X,2HHM/)
 ...
 1310 FORMAT(131H0*NOTE* METHODS VALID FOR CONFIGURATIONS WITH LEADING(H
     1INGE LINE) AND TRAILING EDGES OF THE CONTROL SURFACE SWEPT AHEAD O
     2F THE MACH/8X,34HLINES FROM THE DEFLECTED CONTROLS.)
 1320 FORMAT(103H0*NOTE* OTHER RESTRICTIONS USED IN THE DERIVATION OF TH
     1E METHODS ARE PRESENTED IN VOL. II AND/OR DATCOM)
 1330 FORMAT(126H0*NOTE* CH(A) AND CH(D) DERIVATIVES ARE FOR STRAIGHT SI
     1DED SYMMETRICAL BICONVEX AIRFOILS BASED ON TWICE THE AREA-MOMENT O
     2F THE/8X,29HCONTROL ABOUT ITS HINGE LINE.)
 1340 FORMAT(1H1,28X,48HAUTOMATED STABILITY AND CONTROL METHODS PER APRI
     1,24HL 1976 VERSION OF DATCOM,/,41X,
     248HCHARACTERISTICS OF HIGH LIFT AND CONTROL DEVICES)
 ...
 1470 FORMAT(1H0,12X,46H---------INCREMENTS DUE TO DEFLECTION---------
     1,14X,30H---DERIVATIVES (PER DEGREE)---/1H0,5X,5HDELTA,
     25X,5HD(CL),5X,5HD(CM),4X,9HD(CL MAX),4X,9HD(CD MIN),16X,6H(CLA)D,
     3 5X,5H(CH)A,7X,5H(CH)D//)
 1480 FORMAT(1H0,2X,106H*** NOTE * HINGE MOMENT DERIVATIVES ARE BASED ON
     1 TWICE THE AREA-MOMENT OF THE CONTROL ABOUT ITS HINGE LINE//1H0,12
     2X,83H--------- INDUCED DRAG COEFFICIENT INCREMENT , D(CDI) , DUE T
     3O DEFLECTION ---------)
 ...
 1510 FORMAT(3X,0PF5.1,7X,1PE9.2,1X,E9.2,1X,E9.2,1X,E9.2,1X,E9.2,1X,
     1       E9.2,1X,E9.2,1X,E9.2,1X,E9.2,1X,E9.2)
 ...
 1540 FORMAT( 17H0(DELTAL-DELTAR)=,F5.1,7X,F5.1,7X,F5.1,7X,F5.1,7X,F5.1,
     17X,F5.1,7X,F5.1,7X,F5.1,7X,F5.1,7X,F5.1)
 1550 FORMAT( 6H0ALPHA/1H0)
 1560 FORMAT(1X,0PF5.1,8X,1PE10.3,2X,E10.3,2X,E10.3,2X,E10.3,2X,
     1       E10.3,2X,E10.3,2X,E10.3,2X,E10.3,2X,E10.3,2X,E10.3)
 ...
 1610 FORMAT(1H0,27X, 74H----------YAWING MOMENT COEFFICIENT,CN,DUE TO C
     1ONTROL DEFLECTION----------)
 1620 FORMAT(1H0/
     1       1H0,44X,6HDELTAL,10X,6HDELTAR,10X,8H(CL)ROLL/1H0/(45X,
     2       0PF5.1,11X,F5.1,11X,1PE11.4))
 ...
 1660 FORMAT(1H0,4X,5HALPHA,5X,2HCL,9X,2HCD,7X,2HCM,5X,6HDELTAT,3X,5HD(C
     1L),4X,9HD(CL MAX),5X,6HD(CDI),4X,9HD(CD MIN),3X,5HCH(A),8X,5HCH(D)
     2/1H0)
 1670 FORMAT(1H0,9X,29H----------UNTRIMMED----------,12X,64H------------
     1-----------AT TRIM DEFLECTION-----------------------)
